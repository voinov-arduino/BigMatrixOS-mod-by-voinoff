// Модифицированная адресная матрица для больших эканов с поддержкой большого количества текста
...
#define DEFAULT_AP_NAME "artobject"           // Имя точки доступа по умолчанию 
#define DEFAULT_AP_PASS "53253514"           // Пароль точки доступа по умолчанию
...
#define D_EFFECT_SPEED 60     // скорость эффектов по умолчанию (мс)
...
#define DEMO_TEXT_3              3
#define DEMO_TEXT_4              4
#define DEMO_TEXT_5              5
#define DEMO_TEXT_6              6
#define DEMO_TEXT_7              7
#define DEMO_TEXT_8              8
#define DEMO_TEXT_9              9
#define DEMO_TEXT_10             10
#define DEMO_TEXT_11             11
#define DEMO_TEXT_12             12
#define DEMO_TEXT_13             13
#define DEMO_NOISE_MADNESS       14
#define DEMO_NOISE_CLOUD         15
#define DEMO_NOISE_LAVA          16
#define DEMO_NOISE_PLASMA        17
#define DEMO_NOISE_RAINBOW       18
#define DEMO_NOISE_RAINBOW_STRIP 19
#define DEMO_NOISE_ZEBRA         20
#define DEMO_NOISE_FOREST        21
#define DEMO_NOISE_OCEAN         22
#define DEMO_SNOW                23
#define DEMO_SPARKLES            24
#define DEMO_MATRIX              25
#define DEMO_STARFALL            26
#define DEMO_BALL                27
#define DEMO_BALLS               28
#define DEMO_RAINBOW             29
...
char apName[17] = "artobject";                    // Имя сети в режиме точки доступа
char apPass[9]  = "53253514";                    // Пароль подключения к точке доступа
...
void setup() {
  ...
  */
  ...
  // Текст для демо-режимов "бегущая строка"
  TEXT_1 = String(F("КИРОВСКИЙ РАЙОН В 1872 ГОДУ ПАРОХОДЧИКИ БРАТЬЯ КАМЕНСКИЕ ПРИСМОТРЕЛИ ЗДЕСЬ МЕСТО ДЛЯ МЕЖНОВИГАЦИОННОГО ОТСТОЯ И РЕМОНТА СВОЕГО ФЛОТА"));       // Строка заданного цвета
  TEXT_2 = String(F("В НАЧАЛЕ НЫНЕШНЕЙ УЛИЦЫ ТАНЦОРОВА СУЩЕСТВОВАЛО ТОГДА ОЗЕРО ПОД НАЗВАНИЕМ БАЙКАЛ. НА ЕГО БЕРЕГАХ И ПОСТАВИЛИ ПЕРВЫЕ ДВА ДЕСЯТКА ДОМОВ"));   // Строка с плавной сменой цвета
  TEXT_3 = String(F("ДЗЕРЖИНСКИЙ - КОЛЫБЕЛЬ И ЦЕНТР НАУЧНОЙ МЫСЛИ ПЕРМИ: ИМЕННО ЗДЕСЬ В 1916 ГОДУ БЫЛ ОСНОВАН ПЕРМСКИЙ ГОСУДАРСТВЕННЫЙ УНИВЕРСИТЕТ"));   // Строка с разноцветными буквами
  TEXT_4 = String(F("ИНДУСТРИАЛЬНЫЙ РАЙОН - САМЫЙ МОЛОДОЙ РАЙОН ГОРОДА ПЕРМИ"));
  TEXT_5 = String(F("ЛЕНИНСКИЙ. ВПЛОТЬ ДО 70-Х ГОДОВ XX ВЕКА НЫНЕШНЯЯ ЭСПЛАНАДА БЫЛА КВАРТАЛАМИ МАЛОЭТАЖНОЙ ЗАСТРОЙКИ В ДОЛИНЕ МАЛОЙ РЕКИ ПЕРМЯНКИ"));
  TEXT_6 = String(F("ДО 1938 ГОДУ МОТОВИЛИХА БЫЛА САМОСТОЯТЕЛЬНЫМ ГОРОДОМ МОЛОТОВО КОТОРЫЙ В 1938 ПОЛУЧИЛ НАЗВАНИЕ МОЛОТОВСКИЙ РАЙОН Г. ПЕРМИ"));
  TEXT_7 = String(F("ИСТОРИЯ МИКРОРАЙОНА ЛЕВШИНО ОРДЖОНИКИДЗОВСКОГО РАЙОНА НАЧИНАЕТСЯ С СЕРЕДИНЫ XVII ВЕКА."));
  TEXT_8 = String(F("ТОГДА МЕСТО В КОТОРОМ КРЕСТЬЯНИН ПЕРЕВОЗИЛ ЛЮДЕЙ ЧЕРЕЗ РЕКУ ВПЕРВЫЕ БЫЛО НАЗВАНО ЛЕВШИНЫМ ПЕРЕВОЗОМ"));
  TEXT_9 = String(F("САД ИМЕНИ ГОРЬКОГО В СВЕРДЛОВСКОМ РАЙОНЕ ОБРАЗОВАН В 1804-1805 ГОДАХ."));
  TEXT_10 = String(F("КОГДА В ЮЖНОЙ ЧАСТИ ГОРОДА БЫЛ ВЫКОПАН РОВ НАСЫПАН ВАЛ И ПОСАЖЕНЫ ЧЕТЫРЕ РЯДА БЕРЕЗ ОБРАЗОВАВШИХ СОБОЙ ТРИ АЛЛЕИ ДЛЯ ПЕШЕХОДОВ И ЭКИПАЖЕЙ"));
  TEXT_11 = String(F("СКАМЕЙКА 7 РАЙОНОВ РАЗРАБОТАНА И ВЫПОЛНЕНА ШКОЛЬНИКАМИ И СТУДЕНТАМИ ПЕРМИ УЧАСТНИКАМИ ПРОЕКТА АРХИТЕКТУРНЫЕ ИННОВАЦИИ НА УЛИЦЕ ДЕТСТВА."));
  TEXT_12 = String(F("КАФЕ ПАРОВОЗ ПРИГЛАШАЕТ НА ЧАШКУ ГОРЯЧЕГО ЧАЯ"));
  TEXT_13 = String(F("Проходит конкурс на лучшую анимацию для данного экрана - подробности в группе vk.com/ulicadetstva")); 
...
bool startSmartConfig() {
  ...
  while (smartCount++ < 2) { // Пробуем получить настройки 60 раз
